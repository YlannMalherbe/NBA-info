...
class LeagueData:
    ...
    def __init__(self):
        self._teams:{} = {key: TeamData(key) for key in NBA_TEAMS.keys()}
        self._east_teams = {key: TeamData(key) for key in NBA_TEAMS.keys() 
            if NBA_TEAMS[key].conference == Conference.EAST}
        self._west_teams = {key: TeamData(key) for key in NBA_TEAMS.keys() 
